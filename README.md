...
### [Course 3: Natural Language Processing in TensorFlow](https://www.coursera.org/learn/natural-language-processing-tensorflow/home/welcome)
  NLP course mainly teaches about the how to implement NLP application and associated tips. Tokenization is one of the first step in NLP. While writing these application it is important to keep in mind that NLP does not just caters "English" as a language. There are lot of techniques in NLP which are developed due to work on diverse set of language like handling morphologically heavy or agglutination heavy language. Subword tokenization which makes bit less sense for English is a absolute must for [kichwa](https://en.wikipedia.org/wiki/Agglutination#/media/File:Riksirishkakunapalla.png). Tokenization should be in such a way to minimize OOV (out of vocab) words in validation set/testing set. 
  ...
  I have seen a good >25% increase in accuracy by using embeddings which are trained in larger corpus and have high dimentions (>=300d). This helps in both reducing OOV problem and having good (more accurate) embedding vectors. So be sure of using high dimentional embedding if you are using pretrained embeddings. Also it is imporant to remove stopwords, tokenize (use NLTK regex) , lemmatize (NLTK WordNetLemmatizer etc) and other form of prepocessing. Use of NLTK for these type of task is fairly common.
