...
For the validation set the transformations are not applied. [Keras ImageDataGenerator](https://keras.io/preprocessing/image/)
     ...
     - [Project #4: Horse vs human classificaiton using 1K images + data agumentation.](https://github.com/dpant/TensorFlow/blob/main/Course2/HorseVsHumanDataAgumentation.ipynb)           - Notice that Data Agumentation does not always work. For example the horse vs human applying data agumentation did not yeild good result.So its not a substitute of good quality diverse image data. In this particular both train and validation set have similar type of data (images) so doing data agumentation did not helped. Validating accuracy fluctuate widely here as we are chainging the training set in every epoch by adding random transformations.
